## Octavia documentation
Here lies documentation of the Octavia project.

### Support table
* [Implementation sheet](support/implementation.md)
* [SysEx instructions sheet](support/sysex.md)
* [Targets](support/target.md)

### Implementation
* [Basic messages](impl/basic.md)
* [Control changes](impl/cc.md)
* [RPN/NRPN values](impl/pn.md)

### SysEx documentation
* [Mutual instructions](sysex/mutual.md)
* [Roland MT-32](sysex/mt32.md)
* [KORG 05/X5/N5](sysex/korgOld.md)

### API documentation
* [`state.mjs`: the core processing unit](state/index.md)
* [`basic.mjs`: the basis for visualizers](basic/index.md)
* [`bridge.mjs`: the basis for middleware](bridge/index.md)
* [`disp.mjs`: ready-made example visualizers](disp/index.md)
* [`middle.mjs`: ready-made middleware with real-time translation capabilities](middle/index.md)
