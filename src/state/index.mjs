...
let OctaviaDevice = class OctaviaDevice extends CustomEventSource {
			...
			if (this.polyIndexShrink && this.polyIndexLast > 0) {
				...
				if (isShrunk) {
					...
					if (this.#polyState[newPolyLast + 1] !== 0) {
						console.error(`Polyphonic slot shrinking edge case.`);
					};
				};
			};
		},
	...
	#runChEvent = {
						...
						if (upThis.#mode === 0) {
							if (det.data[1] < 48) {
								...
								if (upThis.#chType[part] > 0) {
									det.data[1] = upThis.getChPrimitive(part, 1, false) /*this.getChCc(part, 0)*/;
								...
								if (det.data[1] > 0) {
									...
									upThis.switchMode("gs", 0);
								};
							} else if (det.data[1] === 56) {
								upThis.switchMode(upThis.#detect.x5 === 82 ? "x5d" : "05rw", 0);
							} else if (det.data[1] === 62) {
								upThis.switchMode(upThis.#detect.x5 === 82 ? "x5d" : "05rw", 0);
							} else if (det.data[1] === 63) {
								upThis.switchMode(modeIdx[upThis.#detect.ds], 0);
							} else if (det.data[1] === 64 || det.data[1] === 127) {
								upThis.switchMode("xg", 0);
							} else if (det.data[1] === 120 || det.data[1] === 121) {
								upThis.switchMode("g2", 0);
							};
		...
		248: function (det) {
			...
			if (this.clockTicker.paused) {
				...
				this.dispatchEvent("tempo", Math.round(25000 / timeDiff) * 0.1);
			};
	...
	setLetter(data, source, offset = 0, delay = 3200) {
		let upThis = this,
		invalidCp;
		...
		if (invalidCp) {
			...
			console.warn(`${source ?? ""}${source ? ": i" : "I"}nvalid code point${invalidCp.length > 1 ? "s" : ""}: 0x${invalidCp.join(", 0x")}`);
		};
	};
	setLetterText(data, source, delay = 3200) {
		let upThis = this;
		if (typeof data !== "string") {
			throw(new TypeError(`${source ?? ""}${source ? ": i" : "I"}nvalid data source, must be a valid string.`))
		};
		upThis.#letterDisp = data;
		upThis.#letterSet = upThis.clockSource.now();
		upThis.#letterExpire = upThis.clockSource.now() + delay;
		upThis.dispatchEvent("letter");
	};
	...
	setPortModeId(port, range, modeId) {
	...
	copyChSetup(sourceCh, targetCh, failWhenActive) {
		...
		let upThis = this;
		if (failWhenActive && upThis.#chActive[targetCh] !== upThis.CH_INACTIVE) {
	...
	runEvent(ingressEvent) {
		...
		switch (ingressEvent.group) {
			...
			default: {
				warningMessage = `Unknown message group "${ingressEvent.group}".`;
			};
		};
	...
	constructor() {
		...
		upThis.#seUnr.add([9], (msg, track, id) => {
			...
			upThis.setPortModeId(upThis.getTrackPort(track), 1, [/*modeMap.gm*/upThis.#detect.gm, modeMap["?"], upThis.#detect.g2][msg[0] - 1]);
		...
		this.#seXg.add([76, 0, 0], (msg, track, id) => {
			switch (msg[0]) {
				...
				case 126: {
					...
					upThis.setPortModeId(upThis.getTrackPort(track), 4, modeMap.xg);
			...
			let offset = msg[0];
			if (offset < 64) {
				upThis.setLetter(msg.subarray(1), "XG letter display", offset);
			} else {
						...
						if (ri < 27) {
							...
							if (ri & 1) {
								if (ri < 23) {
									...
									if (e && e < 96) {
										upThis.assignChAce(part, 130 + pId);
									};
			...
			msg.subarray(1).forEach((e, i) => {
				...
				if (ri === 8) {
					...
					console.debug(`${dPref}default bank: ${["MU Basic", "MU100 Native", "PSR/LE", "QY100"][e] ?? e}.`);
			...
			switch (msg[0]) {
				case 126: {
					...
					upThis.setPortModeId(upThis.getTrackPort(track), 1, modeMap.xg);
					break;
				};
				default: {
			...
			msg.subarray(2).forEach((e, i) => {
				...
				if (ri < 10) {
					...
					if (!opt?.noAce && e > 0 && e !== 64) {
						upThis.assignChAce(part, targetCc);
					};
					upThis.dispatchEvent("cc", {
						part,
						cc: targetCc,
			...
			if (msg[0] === 0) {
				...
				upThis.setPortModeId(upThis.getTrackPort(track), 1, modeMap.qy10);
			...
			if (id === 115) {
				// DOC reset
				upThis.switchMode("doc", 2);
				upThis.setPortModeId(upThis.getTrackPort(track), 1, modeMap.doc);
		...
		this.#seXg.add([39, 48], (msg, track, id) => {
			...
			let offset = msg[0];
			upThis.setLetter(msg.subarray(1), "TG300 letter display", offset);
		...
		this.#seGs.add([66, 18, 0, 0, 127], (msg, track, id) => {
			// GS mode set
			upThis.switchMode("sc", 2);
			upThis.setPortModeId(upThis.getTrackPort(track), 2, modeMap.sc);
			...
			switch (msg[0]) {
				case 127: {
					// Roland GS reset
					upThis.switchMode("gs", 2);
					upThis.setPortModeId(upThis.getTrackPort(track), 4, modeMap.gs);
					/*upThis.setChCc(9, 0, 120);
					upThis.setChCc(25, 0, 120);
					upThis.setChCc(41, 0, 120);
					upThis.setChCc(57, 0, 120);*/
			...
			switch (msg[0]) {
				case 0: {
					// GS display letter
					let offset = msg[1];
					upThis.setLetter(msg.subarray(2), "GS display text", offset);
		...
		this.#seAi.add([54, 65], (msg, track) => {
			...
			upThis.switchMode(x5Target, 1);
			upThis.setPortModeId(upThis.getTrackPort(track), 1, modeMap[x5Target]);
			...
			upThis.setPortModeId(port, 1, modeMap[x5Target]);
		...
		this.#seGs.add([22, 18, 127], (msg, track, id) => {
			...
			upThis.setPortModeId(upThis.getTrackPort(track), 1, modeMap.mt32);
			upThis.#modeKaraoke = upThis.KARAOKE_NONE;
			upThis.userBank.clearRange({msb: 49, lsb: 127, prg: [0, 127]});
			upThis.modelEx.mt32.writeTimbre = true;
		...
		this.#seAi.add([66, 0], (msg, track) => {
			// Mode switch
			upThis.switchMode("ns5r", 1);
			upThis.setPortModeId(upThis.getTrackPort(track), 2, modeMap.ns5r);
			upThis.#modeKaraoke = upThis.KARAOKE_NONE;
			...
			upThis.switchMode(n5Target, 1);
			upThis.setPortModeId(upThis.getTrackPort(track), 2, modeMap[n5Target]);
			upThis.#modeKaraoke = upThis.KARAOKE_NONE;
			...
			if (offset < 32) {
				// Letter display
				upThis.setLetter(msg.subarray(1, 33), "NS5R letter display");
			} else {
				// Bitmap display
			...
			upThis.setPortModeId(port, 2, modeMap.ns5r);
		...
		upThis.#seKg.add([16, 0, 8, 0], (msg, track, id) => {
			...
			([() => {
				// GMega bank set
				upThis.switchMode("k11", 2);
				upThis.setPortModeId(upThis.getTrackPort(track), 2, modeMap.k11);
				upThis.#modeKaraoke = upThis.KARAOKE_NONE;
			...
			switch (msg[0]) {
				case 0: {
					...
					switch (msg[1]) {
						...
						case 127: {
							// SG reset
							upThis.switchMode("sg", 2);
							upThis.setPortModeId(upThis.getTrackPort(track), 1, modeMap.sg);
							break;
						};
					};
					break;
		...
		this.#seXg.add([127, 0], (msg, track, id) => {
			...
			//upThis.setPortModeId(port, 2, modeMap.s90es);
			let part = upThis.chRedir(msg[0], track, true),
			chOff = ccOffTable[part],
			offset = msg[1];
			if (msg[0] > 15) {
		...
		upThis.#seXg.add([100, 14], (msg, track, id) => {
			...
			upThis.setPortModeId(upThis.getTrackPort(track), 1, modeMap.cs6x);
			// CS6x bulk start
				...
				case upThis.EXT_DX: {
					...
					msg.subarray(1).forEach((e, i) => {
						...
						if (ri < 6) {
							...
							let targetCc = ri + 142;
							upThis.setChCc(part, targetCc, e);
							e !== 64 && upThis.assignChAce(part, targetCc);
						} else if (ri < 12) {
							// Modulator level 1~6
							let targetCc = ri + 144;
							upThis.setChCc(part, targetCc, e);
		...
		this.#seGs.add([0, 72, 18, 0, 0, 0, 0], (msg, track, id) => {
			...
			upThis.switchMode("sd", 2);
			upThis.setPortModeId(upThis.getTrackPort(track), 2, modeMap.sd);
		...
		upThis.#seAi.add([0, 1, 73, 78], (msg, track, id) => {
			upThis.switchMode("krs", 1);
			upThis.setPortModeId(upThis.getTrackPort(track), 1, modeMap.krs);
		...
		let an1xAssign = function (e, track, slot) {
			let part = upThis.chRedir(e, track, true);
			upThis.setChModeId(part, modeMap.an1x);
			upThis.pushChPrimitives(part);
